...
- Improve ``ScopedAllocator::ScopedAllocator()`` alignment validation. The alignment is now
...
- Add ``TaskContext::get_task_stream()`` to retrieve the current task's active CUDA stream.
- Deprecate ``TaskRegistrar::record_task(std::int64_t std::unique_ptr<TaskInfo>)``. Users
  should use ``LegateTask::Registrar`` combined with ``TaskRegistrar::register_all_tasks()``
  instead.
...
- Move the implementation detail of the CPU communicator (i.e.,
  ``legate::comm::coll::BackendNetwork``,
