...
def write_array(
    ...
    store_shape = tuple(store.shape)
    store_dtype_np = store.type.to_numpy_dtype()
    _raise_if_unsupported_zarr_dtype(store_dtype_np)
    store_dtype = str(store_dtype_np)
    ...
    if chunks is None:
        ...
        chunks_arg = (int(chunks),) * len(store_shape)
    else:
        try:
            chunks_arg = tuple(
    ...
    if padded:
        ...
        padded_st.fill(0)
        sliced = padded_st[tuple(map(slice, metadata.shape))]
        runtime.issue_copy(sliced, store)
        store = padded_st
    tile.to_tiles(path=dirpath, store=store, tile_shape=metadata.chunks)
...
def read_array(dirpath: Path | str) -> LogicalStore:
    ...
    ret = tile.from_tiles(
        path=dirpath,
        shape=shape,
        store_type=Type.from_numpy_dtype(metadata.dtype),
