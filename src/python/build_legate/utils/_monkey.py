...
import scikit_build_core.build.wheel  # type: ignore
from scikit_build_core.build._editable import (  # type: ignore
    editable_redirect,
    libdir_to_installed,
    mapping_to_modules,
)
from scikit_build_core.build._wheelfile import WheelWriter  # type: ignore
from scikit_build_core.settings.skbuild_model import (  # type: ignore
    ScikitBuildSettings,
)


def patched_make_editable(  # type: ignore
    *,
    ...
    # Our patch
    if False and settings.wheel.install_dir.startswith("/"):  # type: ignore
...
def monkey_patch_skbuild_editable() -> None:
    ...
    assert callable(scikit_build_core.build.wheel._make_editable)
    scikit_build_core.build.wheel._make_editable = patched_make_editable
