                                                ...
                                                std::integer_sequence<index_type, Is...>)
{
  ...
  if constexpr (DIM == extents_type::rank() - 1) {  // reached bottom
    (static_cast<void>(std::forward<F>(fn)(indices..., Is)), ...);
  } else {
    (loop_dispatch_<DIM + 1>(extents, std::forward<F>(fn), indices..., Is), ...);
  }
}
                                                 ...
                                                 Indices... indices)
{
  ...
  for (index_type i = 0; i < dim_ext; ++i) {
    if constexpr (DIM == extents_type::rank() - 1) {  // reached bottom
      static_cast<void>(std::forward<F>(fn)(indices..., i));
    } else {
      loop_dispatch_<DIM + 1>(extents, std::forward<F>(fn), indices..., i);
    }
  }
}
