 ...
 public:
                         ...
                         Legion::Runtime* runtime)
  {
    ...
    LEGATE_CHECK(coll::BackendNetwork::get_network()->comm_type ==
                 legate::comm::coll::CollCommType::CollUCC);

    int mpi_rank;
    // at the moment, legate only support ucc with mpi, this will be removed in the future
    // with different process management and oob allgather
    LEGATE_CHECK_MPI(legate::detail::comm::mpi::detail::MPIInterface::mpi_comm_rank(
      legate::detail::comm::mpi::detail::MPIInterface::MPI_COMM_WORLD(), &mpi_rank));
    return mpi_rank;
  }
