...
ListArrayAccessor<READ_ONLY, VAL>::ListArrayAccessor(ListArray array)
{
...
template <typename VAL>
ListArrayAccessor<READ_ONLY, VAL>::~ListArrayAccessor()
{
}

template <typename VAL>
Span<const VAL> ListArrayAccessor<READ_ONLY, VAL>::operator[](const Point<1>& p)
{
...
template <typename VAL>
ListArrayAccessor<WRITE_DISCARD, VAL>::ListArrayAccessor(ListArray array)
{
...
template <typename VAL>
ListArrayAccessor<WRITE_DISCARD, VAL>::~ListArrayAccessor()
{
...
template <typename VAL>
void ListArrayAccessor<WRITE_DISCARD, VAL>::insert(const legate::Span<const VAL>& value)
{
  check_overflow();
...
template <typename VAL>
void ListArrayAccessor<WRITE_DISCARD, VAL>::check_overflow()
{
...
StringArrayAccessor<READ_ONLY>::StringArrayAccessor(StringArray array)
  : ListArrayAccessor(array.as_list_array())
{
}

std::string_view StringArrayAccessor<READ_ONLY>::operator[](const Point<1>& p)
{
  auto span = ListArrayAccessor::operator[](p);
  return std::string_view(reinterpret_cast<const char*>(span.ptr()), span.size());
}

StringArrayAccessor<WRITE_DISCARD>::StringArrayAccessor(StringArray array)
  : ListArrayAccessor(array.as_list_array())
{
}

void StringArrayAccessor<WRITE_DISCARD>::insert(const std::string& value)
{
  ListArrayAccessor::insert(
    legate::Span<const int8_t>(reinterpret_cast<const int8_t*>(value.data()), value.size()));
}

void StringArrayAccessor<WRITE_DISCARD>::insert(const std::string_view& value)
{
  ListArrayAccessor::insert(
    legate::Span<const int8_t>(reinterpret_cast<const int8_t*>(value.data()), value.size()));
}
