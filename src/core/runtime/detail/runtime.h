...
class Runtime {
 public:
  ...
  void record_reduction_operator(uint32_t type_uid, int32_t op_kind, int32_t legion_op_id);
  [[nodiscard]] int32_t find_reduction_operator(uint32_t type_uid, int32_t op_kind) const;
 ...
 public:
  ...
  template <class T>
  [[nodiscard]] T get_tunable(Legion::MapperID mapper_id, int64_t tunable_id);
  template <class T>
  [[nodiscard]] T get_core_tunable(int64_t tunable_id);
  ...
  [[nodiscard]] Legion::Future extract_scalar(const Legion::Future& result, uint32_t idx) const;
