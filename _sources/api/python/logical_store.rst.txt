...
`LogicalStore` is a multi-dimensional data container for fixed-size elements.
Stores are internally partitioned and distributed across the system. By
default, Legate clients need not create nor maintain the partitions explicitly,
and the Legate runtime is responsible for managing them. Legate clients can
control how stores should be partitioned for a given task by attaching
partitioning constraints to the task (see section :ref:`label_operation` for
...
physical allocation (see section TODO for details about
inline allocations).
