...
TEST_F(ProcessorRangeTest, Create)
{
  constexpr legate::mapping::ProcessorRange range{
    /*low_id=*/1, /*high_id=*/3, /*per_node_proc_count=*/0};
...
TEST_F(ProcessorRangeTest, CreateEmpty)
{
  ...
  constexpr legate::mapping::ProcessorRange range1{
    /*low_id=*/1, /*high_id=*/0, /*per_node_proc_count=*/1};
  check_empty(range1);

  constexpr legate::mapping::ProcessorRange range2{
    /*low_id=*/3, /*high_id=*/3, /*per_node_proc_count=*/0};
  check_empty(range2);
}
...
{
  constexpr legate::mapping::ProcessorRange range1{
    /*low_id=*/2, /*high_id=*/6, /*per_node_proc_count=*/2};
  constexpr legate::mapping::ProcessorRange range2{
    /*low_id=*/2, /*high_id=*/6, /*per_node_proc_count=*/2};
  ...
  constexpr legate::mapping::ProcessorRange range3{
    /*low_id=*/1, /*high_id=*/6, /*per_node_proc_count=*/2};
  static_assert(!(range1 == range3));
  static_assert(range1 != range3);
  static_assert(range3 < range1);
  static_assert(!(range1 < range3));

  constexpr legate::mapping::ProcessorRange range4{
    /*low_id=*/2, /*high_id=*/5, /*per_node_proc_count=*/2};
  static_assert(!(range1 == range4));
  static_assert(range1 != range4);
  static_assert(range4 < range1);
  static_assert(!(range1 < range4));

  constexpr legate::mapping::ProcessorRange range5{
    /*low_id=*/2, /*high_id=*/6, /*per_node_proc_count=*/1};
...
TEST_F(ProcessorRangeTest, IntersectionOperator)
{
  ...
  constexpr legate::mapping::ProcessorRange range1{
    /*low_id=*/0, /*high_id=*/3, /*per_node_proc_count=*/1};
  constexpr legate::mapping::ProcessorRange range2{
    /*low_id=*/2, /*high_id=*/4, /*per_node_proc_count=*/1};
  constexpr auto result1 = range1 & range2;
  static_assert(result1 == legate::mapping::ProcessorRange{
                             /*low_id=*/2, /*high_id=*/3, /*per_node_proc_count=*/1});
  ...
  static_assert(result2 == legate::mapping::ProcessorRange{
                             /*low_id=*/2, /*high_id=*/3, /*per_node_proc_count=*/1});

  // Generate empty range
  constexpr legate::mapping::ProcessorRange range5{
    /*low_id=*/0, /*high_id=*/2, /*per_node_proc_count=*/1};
  constexpr legate::mapping::ProcessorRange range6{
    /*low_id=*/3, /*high_id=*/5, /*per_node_proc_count=*/1};
  constexpr auto result3 = range5 & range6;
  static_assert(result3 == legate::mapping::ProcessorRange{
                             /*low_id=*/0, /*high_id=*/0, /*per_node_proc_count=*/1});
...
TEST_F(ProcessorRangeTest, NegativeIntersectionOperator)
{
  constexpr legate::mapping::ProcessorRange range1{
    /*low_id=*/1, /*high_id=*/3, /*per_node_proc_count=*/1};
  constexpr legate::mapping::ProcessorRange range2{
    /*low_id=*/2, /*high_id=*/4, /*per_node_proc_count=*/2};
...
{
  constexpr legate::mapping::ProcessorRange range{
    /*low_id=*/0, /*high_id=*/7, /*per_node_proc_count=*/2};
...
TEST_F(ProcessorRangeTest, Slice)
{
  ...
  constexpr legate::mapping::ProcessorRange range1{
    /*low_id=*/3, /*high_id=*/1, /*per_node_proc_count=*/1};
  static_assert(range1.slice(/*from=*/0, /*to=*/0).count() == 0);
  static_assert(range1.slice(/*from=*/4, /*to=*/6).count() == 0);

  // Slice nonempty range with empty range
  constexpr legate::mapping::ProcessorRange range2{
    /*low_id=*/1, /*high_id=*/3, /*per_node_proc_count=*/1};
  static_assert(range2.slice(/*from=*/0, /*to=*/0).count() == 0);
  static_assert(range2.slice(/*from=*/4, /*to=*/6).count() == 0);
  static_assert(range2.slice(/*from=*/1, /*to=*/0).count() == 0);
