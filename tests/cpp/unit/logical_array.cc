...
void test_string_array(bool nullable)
{
  ...
  // Bound
  {
    ...
    EXPECT_TRUE(array.unbound());
    EXPECT_EQ(array.dim(), 1);
    EXPECT_EQ(array.extents().data(), (std::vector<size_t>{5}));
...
void test_promote(bool nullable)
{
  ...
  // primitive array
  {
    ...
    EXPECT_EQ(promoted.extents().data(), (std::vector<size_t>{5, 1, 2, 3, 4}));

    // Note: gitlab issue #16 unexpected behavior
    promoted = bound_array.promote(2, -1);
    EXPECT_EQ(promoted.extents().data(),
              (std::vector<size_t>{1, 2, static_cast<size_t>(-1), 3, 4}));
  ...
  // struct array
  {
    ...
    EXPECT_EQ(promoted.extents().data(), (std::vector<size_t>{4, 5, 10, 6}));

    // Note: gitlab issue #16 unexpected behavior
    promoted = bound_array.promote(1, -1);
    EXPECT_EQ(promoted.extents().data(), (std::vector<size_t>{4, static_cast<size_t>(-1), 5, 6}));
...
void test_project(bool nullable)
{
  ...
  // primitive array
  {
    ...
    EXPECT_EQ(projected.extents().data(), (std::vector<size_t>{2, 3, 4}));
  ...
  // struct array
  {
    ...
    EXPECT_EQ(projected.extents().data(), (std::vector<size_t>{5, 6}));
...
void test_slice(bool nullable)
{
  ...
  // primitive array
  {
    ...
    EXPECT_EQ(sliced.extents().data(), (std::vector<size_t>{1, 2, 1, 4}));
    ...
    EXPECT_EQ(sliced.extents().data(), (std::vector<size_t>{1, 2, 3, 4}));
  ...
  // struct array
  {
    ...
    // slice [OPEN, STOP) of dim i
    auto sliced = bound_array.slice(2, legate::Slice(-2, -1));
    EXPECT_EQ(sliced.extents().data(), (std::vector<size_t>{4, 5, 1}));

    sliced = bound_array.slice(2, legate::Slice(1, 2));
    EXPECT_EQ(sliced.extents().data(), (std::vector<size_t>{4, 5, 1}));

    sliced = bound_array.slice(2, legate::Slice(1, 4));
    EXPECT_EQ(sliced.extents().data(), (std::vector<size_t>{4, 5, 3}));
    ...
    // full slice
    sliced = bound_array.slice(0, legate::Slice());
    EXPECT_EQ(sliced.extents().data(), (std::vector<size_t>{4, 5, 6}));
...
void test_transpose(bool nullable)
{
  ...
  // primitive array
  {
    ...
    EXPECT_EQ(transposed.extents().data(), (std::vector<size_t>{2, 1, 4, 3}));
  ...
  // struct array
  {
    ...
    EXPECT_EQ(transposed.extents().data(), (std::vector<size_t>{5, 4, 6}));
