...
{
  auto transform =
    legate::make_internal_shared<legate::detail::Transpose>(std::vector<std::int32_t>({2, 1, 0}));
  auto color    = transform->convert_color(legate::tuple<std::uint64_t>{1, 2, 3});
...
{
  auto transform =
    legate::make_internal_shared<legate::detail::Transpose>(std::vector<std::int32_t>({-1, 0}));

  if (LEGATE_DEFINED(LEGATE_USE_DEBUG)) {
    ASSERT_THAT(
      [&] { static_cast<void>(transform->convert_color(legate::tuple<std::uint64_t>{2, 1})); },
...
TEST_F(TransformTransposeUnit, TransposeInvertColor)
{
  auto transform = legate::make_internal_shared<legate::detail::Transpose>(
    std::vector<std::int32_t>({4, 1, 0, 3, 2}));
  auto color    = transform->invert_color(legate::tuple<std::uint64_t>{4, 1, 0, 3, 2});
  auto expected = legate::tuple<std::uint64_t>{0, 1, 2, 3, 4};
...
{
  auto transform =
    legate::make_internal_shared<legate::detail::Transpose>(std::vector<std::int32_t>({0}));

  if (LEGATE_DEFINED(LEGATE_USE_DEBUG)) {
    ASSERT_THAT(
      [&] { static_cast<void>(transform->invert_color(legate::tuple<std::uint64_t>{1, 2})); },
      ::testing::ThrowsMessage<std::out_of_range>(
        ::testing::HasSubstr("mapping size 1 != tuple size 2")));
  }
}
