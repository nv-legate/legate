...
constexpr float FLOAT_VALUE          = 42.0F;
...
void test_write_accessor_by_task(legate::LogicalStore& logical_store, legate::Scalar& scalar)
{
  auto runtime = legate::Runtime::get_runtime();
  auto context = runtime->find_library(Config::LIBRARY_NAME);
  auto task    = runtime->create_task(context, WriteAccessorTestTask::TASK_CONFIG.task_id());
...
std::vector<std::tuple<legate::Shape, legate::Type, legate::Scalar>> write_accessor_cases()
{
  std::vector<std::tuple<legate::Shape, legate::Type, legate::Scalar>> cases = {
    ...
    {legate::Shape{2, 2}, legate::bool_(), legate::Scalar{true}},
    {legate::Shape{1, 2, 4}, legate::float16(), legate::Scalar{static_cast<__half>(FLOAT_VALUE)}},
    {legate::Shape{1, 4, 2, 3}, legate::float32(), legate::Scalar{float{FLOAT_VALUE}}},
  };

#if LEGATE_MAX_DIM >= 5
  cases.emplace_back(
    legate::Shape{1, 2, 5, 3, 4}, legate::float64(), legate::Scalar{double{FLOAT_VALUE}});
#endif
#if LEGATE_MAX_DIM >= 6
...
TEST_P(BoundStoreWriteAccessorTest, BoundStore)
{
  ...
  test_write_accessor_by_task(logical_store, scalar);
}
