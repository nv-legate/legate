...
TEST_P(ConfigureSysMemUnit, Preset)
{
  ...
  const auto numamem         = NUMAMemType{nullptr, "--numamem", ScaledType{1, MB}};
  auto sysmem                = SysMemType{nullptr, "--sysmem", ScaledType{SYSMEM_SIZE, MB}};
...
TEST_F(ConfigureSysMemUnit, NoAutoConfig)
{
  ...
  const auto core            = MockCoreModuleConfig{1, /* should_fail */ false};
  const auto numamem         = NUMAMemType{nullptr, "--numamem", ScaledType{0, MB}};
  auto sysmem                = SysMemType{nullptr, "--sysmem", ScaledType{-1, MB}};
...
TEST_F(ConfigureSysMemUnit, NUMAMemAllocated)
{
  constexpr auto MINIMAL_MEM = 256;
  const auto core            = MockCoreModuleConfig{1, /* should_fail */ false};
  const auto numamem         = NUMAMemType{nullptr, "--numamem", ScaledType{100, MB}};
  auto sysmem                = SysMemType{nullptr, "--sysmem", ScaledType{-1, MB}};
