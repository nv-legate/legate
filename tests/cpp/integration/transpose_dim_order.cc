...
// TODO(amberhassaan): uncomment this test when
// https://github.com/nv-legate/legate.internal/issues/2417
// is fixed.
TEST_P(TransposeDimOrder, EmptyPromoteTranspose)
{
  GTEST_SKIP() << "Disable until https://github.com/nv-legate/legate.internal/issues/2417 is fixed";
  auto runtime             = legate::Runtime::get_runtime();
  auto store               = runtime->create_store(legate::Shape{}, legate::int64());
  const bool fortran_order = GetParam();
