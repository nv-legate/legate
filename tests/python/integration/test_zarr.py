...
class TestZarrV2:
    ...
    def test_write_scalar_array(self, tmp_path: Path) -> None:
        ...
        store = get_legate_runtime().create_store(ty.float64, ())

        with pytest.raises(
            ValueError,
            match="Cannot write a 0-dimensional \\(scalar\\) array to Zarr",
        ):
            write_array(st=store, dirpath=tmp_path)
    ...
    def test_write_array_default_chunks_fixed_expectations(
        ...
        write_array(st=store, dirpath=actual_path, chunks=None)
        get_legate_runtime().issue_execution_fence(block=True)
    ...
    def test_write_array_minus_one_chunks_are_not_default(
        ...
        write_array(st=store, dirpath=default_path, chunks=None)
        get_legate_runtime().issue_execution_fence(block=True)
        assert _read_zarr_chunks(default_path) == expected_default_chunks

        full_extent_path = tmp_path / "full-extent"
        write_array(st=store, dirpath=full_extent_path, chunks=(-1,))
        get_legate_runtime().issue_execution_fence(block=True)
    ...
    def test_write_array_invalid_chunks_rejected(
        ...
        with pytest.raises(exc_type, match=match):
            write_array(st=store, dirpath=tmp_path, chunks=cast(Any, chunks))
    ...
    def test_write_array_overwrites_existing_array(
        ...
        write_array(st=first, dirpath=tmp_path, chunks=(2, 2))
        get_legate_runtime().issue_execution_fence(block=True)
        ...
        write_array(st=second, dirpath=tmp_path, chunks=(3, 2))
        get_legate_runtime().issue_execution_fence(block=True)
    ...
    def test_read_array(
        ...
        b = np.asarray(store.get_physical_store())
        assert_array_equal(a, b)
